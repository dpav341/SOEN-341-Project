...
socket.emit("join room", channel, (prevMessages) => {
  prevMessages.forEach((msg) => addMessage(msg.chatName, msg));
});
...
export default function Channels() {
  ...
  useEffect(() => {
    if (!selectedChannel) return; // Do nothing if no channel is selected

    // Fetch previous messages from MongoDB when selecting a channel
    ...
    return () => {
      socket.off("new message", messageListener); // Cleanup when component unmounts
    };
  }, [selectedChannel]);
