...
const useChatStore = create((set) => ({
  ...
  addReaction: (channel, messageIndex, emoji) => set((state) => {
    const updatedMessages = [...state.messages[channel]];
    updatedMessages[messageIndex].reactions = [
      ...(updatedMessages[messageIndex].reactions || []),
      emoji,
    ];
    return { messages: { ...state.messages, [channel]: updatedMessages } };
  }),

}));

export default function Channels() {
  const { channels, messages, joinChannel, addMessages, addMessage, deleteMessage, addChannel, deleteChannel, addReaction } = useChatStore();
            ...
            <div className="chat-messages">
              {messages[selectedChannel]?.map((msg, idx) => (
                <div key={idx} className={`message ${msg.sender === username ? "my-message" : "other-message"}`}>
                  <button onClick={() => togglePicker(idx)} className="add-channel">
                    <FontAwesomeIcon icon={faPlus} />
                  </button>
                  <div className={reactOn ? 'd-block' : 'd-none'}>
                    <Picker reactionsDefaultOpen={true} onEmojiClick={(emojiObject) => handleReaction(emojiObject.emoji, idx)} />
