...
app.get("/Channels/:roomName", async (req, res) => {
  ...
  try {
    // Fetch messages from the correct collection (no 'channel' field exists)
    const messages = await Conversation.find({ channel: roomName }).sort({ time: 1 }).exec();

    // Format response correctly
    const formattedMessages = messages.map(msg => ({
      sender: msg.name,      // Use 'name' instead of 'sender'
      content: msg.text,     // Use 'text' instead of 'content'
      timeStamp: msg.time,   // Use 'time' instead of 'timeStamp'
    }));
...
io.on("connection", (socket) => {
  ...
  socket.on('newMessage', async (msg) => {
    try {
      ...
      if (!msg.name || !msg.text || !msg.channel) {
      ...
      const newMessage = new Conversation({
        ...
        text: msg.text,
        channel: msg.channel,
        time: new Date(), // Optional: Ensure timestamp is set
      });
