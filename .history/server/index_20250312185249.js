...
app.get("/Channels/:roomName", async (req, res) => {
  ...
  try {
    // Query ALL messages (since there's no `channel` field)
    const messages = await Conversation.find({}).sort({ time: 1 }).exec();

    // Map MongoDB fields to the expected response format
    const formattedMessages = messages.map(msg => ({
      sender: msg.name,      // Convert `name` → `sender`
      content: msg.text,     // Convert `text` → `content`
      timeStamp: msg.time,   // Convert `time` → `timeStamp`
    }));
