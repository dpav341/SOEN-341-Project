...
app.get("/Channels/:roomName", async (req, res) => {
  ...
  try {
    const messages = await Conversation.find({}).sort({ time: 1 }).exec(); // Use `time`, not `timeStamp`

    // Transform MongoDB field names to match frontend expectations
    const formattedMessages = messages.map(msg => ({
      sender: msg.name,      // Map `name` → `sender`
      content: msg.text,     // Map `text` → `content`
      timeStamp: msg.time,   // Map `time` → `timeStamp`
    }));
